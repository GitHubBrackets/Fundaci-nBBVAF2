 ...
 * Contains the implementations of the method validatePermissionsLdap to consume the services with the App75
 * This class inplement APO to validate logged out users.
 * 
...
public class ValidationAccessLdapFrontImpl implements ValidationAccessLdapFront {
	private static final Logger log = Logger.getLogger(ValidationAccessLdapFrontImpl.class);

  	/**      
    *
    * Method validatePermissionsLdap.
    * Contains the parameters for get the URL for the services with the App75
    ...
    */
	...
	public ResponseEntity<?> validatePermissionsLdap(ProceedingJoinPoint pjp){		
		...
		try{
			...
			log.info("IV-USER validatePermissionsLdap = "+request.getRequest().getSession().getAttribute("iv-user"));
			...
			if(request.getRequest().getSession().getAttribute("iv-user") != null && 
			   ...
			   !request.getRequest().getSession().getAttribute("iv-user").toString().toLowerCase().equals("unauthenticated")){
				...
				if(request.getRequest().getSession().getAttribute("tsec") == null){
